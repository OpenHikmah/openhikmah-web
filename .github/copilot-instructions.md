...
- **Theological standards** (binding on any prompt, divine-name, or verse-connection change): stay within the Maturidi/Hanafi tradition; maintain strict Tanzih (never imply physical form or spatial location for divine attributes — see `lib/ai/theological-constraints.ts`'s `TANZIH_CONSTRAINT`, applied via `lib/ai/connection-generator.ts`'s `tanzihDirective()`); never fabricate a Quran verse reference; treat Quranic text as sacred data even in test fixtures (no "lorem ipsum" for Arabic fields); attribute translations correctly (`en.sahih` / Saheeh International from alquran.cloud).
