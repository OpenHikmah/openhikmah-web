...
3. **Use verified verse references** — never fabricate a Quran verse reference. References are checked against the local seeded Quran corpus (`lib/quran/quran-corpus.ts`) first, falling back to a live alquran.cloud fetch only when the corpus lookup misses; if a model returns a reference that resolves nowhere, the system rejects it. In tests, use known valid refs (e.g. `2:255`, `1:1`, `112:1`).
